...
/**
 * Changing interval length, while data to late.
 ...
 * interval 15s). All data, which will be received after the change of the interval length, should be assigned to the
 * new interval length.<br>
 * Precondition: Infrastructure is filled, algorithm and infrastructure parameter are set<br>
 * Requirements:
 */
@SuppressWarnings("unchecked")
public class TC9c_ChangeIntervalLengthLateDataTest {
    
    private static final Logger LOG = LoggerFactory.getLogger(TC9c_ChangeIntervalLengthLateDataTest.class);
    ...
    public TC9c_ChangeIntervalLengthLateDataTest() {
    ...
    public void testChangeIntervalLength() {
        ...
        testStep1(time1315, time1328); //id71 data come in, id73 not
        
        mockClock.advanceBySeconds(1);
        Instant time1329= Instant.now(mockClock);      // 14:13:29
        LOG.debug("---------------------------- " + DTF.format(time1329) + "-----------------------------");
        testStep2(time1329); //interval length for both ids is changed to 30s
        ...
        Instant time1341 = Instant.now(mockClock);      // 14:13:30
        LOG.debug("---------------------------- " + DTF.format(time1341) + "-----------------------------");
        testStep3(time1315, time1341);  //id73 data come in too late
...
//        //interval end (30) reached;
    ...
    public void testStep2(Instant processingTime) {
        Map<String, InfraParameter> newInfraParams = new HashMap<>() {{
                ...
                put(id_73, new InfraParameter(id_73, Type.DIRECT_SET, processingTime, secondsI30, version, defaultvArithmetical, categoryBoudariesPkw, categoryBoudariesLkw));
        }};
     ...
     * @param intervalEnd
     */
    public void testStep3(Instant eventtime, Instant processingTime) {
        List<Object> builders = new ArrayList<>();
        List<InfraState> infraStates = new ArrayList<>();
        ...
        //publish because interval15 is complete
        Mockito.verify(pubMock).publishDiscardedData(argInput.capture(), infraPara.capture(),
                infraStatesArg.capture());
        //value for id73 is discarded, because it came in after interval length change (but for the old interval 15)
    ...
    public void triggerTimeoutEnd(Instant intervalEnd, Instant timeoutEnd, List<Integer> intervalLenghts) {
        ...
        // Both interval 30 values are missing
        assertTrue(argInput.getValue().isEmpty());
        ...
        assertTrue(argMissingIds.getValue()
                .contains(new Tuple3<String, Instant, Integer>(id_73, intervalEnd, secondsI30)));
    }
    //-----------------------------------------------------------------------------------
