...
/**
 * Changing interval length and missing another data.
 ...
 * interval 15s). After the interval change no data come in, so that the interval 15 value will be published, but no
 * more publish's will be done for the interval 30.<br>
 * Precondition: Infrastructure is filled, algorithm and infrastructure parameter are set<br>
 * Requirements:
 */
@SuppressWarnings("unchecked")
public class TC9a_ChangeIntervalLengthMissingDataTest {
    
    private static final Logger LOG = LoggerFactory.getLogger(TC9a_ChangeIntervalLengthMissingDataTest.class);
    ...
    public TC9a_ChangeIntervalLengthMissingDataTest() {
    ...
    public void testChangeIntervalLength() {
        ...
        testStep1(time1300, time1313); //id71 sends values, id73 not
        
        mockClock.advanceBySeconds(1);
        Instant time1314 = Instant.now(mockClock);      // 14:13:26
        LOG.debug("---------------------------- " + DTF.format(time1314) +" -----------------------------");
        testStep2(time1314); //change both interval lengths to 30s
        
        mockClock.advanceBySeconds(1);
        Instant time1315 = Instant.now(mockClock);      // 14:13:15
        LOG.debug("---------------------------- " + DTF.format(time1315) +" -----------------------------");
        triggerIntervalEnd(time1315, Arrays.asList(secondsI15));
        
        mockClock.advanceBySeconds(12);
        Instant time1327 = Instant.now(mockClock);      // 14:13:27
        LOG.debug("---------------------------- " + DTF.format(time1327) +" -----------------------------");
        triggerTimeoutEnd(time1315, time1327, Arrays.asList(secondsI15));
    ...
    public void testStep3(Instant eventtime, Instant intervalEnd) {
        ...
        // No more publish's, because there are no data to publish
    ...
    public void triggerTimeoutEnd(Instant intervalEnd, Instant timeoutEnd, List<Integer> intervalLenghts) {
        ...
        //Interval 15 id71 value came in -> publish. No more ids expected -> no missing ids.
