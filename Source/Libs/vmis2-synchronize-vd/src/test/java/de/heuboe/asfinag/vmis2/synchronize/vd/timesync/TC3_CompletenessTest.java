 ...
 * "Prüffall 3: Test auf Vollständigkeit"
 *
 * Objective: Accepting input data received in several steps<br>
 * Description: The received values are not published, until all measurement points sent data for an interval (or the
 * timeout is reached).<br>
 * Precondition: Infrastructure is filled, algorithm and infrastructure parameter are set<br>
 * Requirements:
 */
@SuppressWarnings("unchecked")
public class TC3_CompletenessTest {
    
    private static final Logger LOG = LoggerFactory.getLogger(TC3_CompletenessTest.class);
    ...
    public void testCompleteness() {
        ...
        testStep1Completeness(time1245, time1230, time1300);
        
        //Missing value sent for interval 15
        mockClock.advanceBySeconds(5);
        Instant time1305 = Instant.now(mockClock);      // 14:13:05
        LOG.debug("---------------------------- " + DTF.format(time1305) +" -----------------------------");
        testStep2Completeness(time1245, time1305);
        ...
        testStep3Completeness(time1312, time1300);
        
        //Values for next interval 15 sent
        testStep4Completeness(time1300, time1312);
        ...
        testStep5Completeness(time1245, time1315);
    ...
    public void testStep1Completeness(Instant eventtime15, Instant eventtime30, Instant processingTime) {
        ...
        // Send data at second 0, no infrastructure states are set => data is discarded
        ...
        // Trigger interval end for interval 15 and interval 30 at second 0, no infrastructure states are
        // set => trigger has no affect
        ...
        // Because the data was discarded and the trigger had no effect, no data is published.
        Mockito.verify(pubMock, Mockito.times(0)).publish(argInput.capture(), argMissingIds.capture(), infraPara.capture(),
        ...
        algo.intervalEndTrigger(processingTime, Arrays.asList(secondsI15, secondsI30));
        Mockito.verify(pubMock, Mockito.times(1)).publish(argInput.capture(), argMissingIds.capture(), infraPara.capture(),
                infraStatesArg.capture(), slowestVehData.capture());
     ...
     */
    public void testStep2Completeness(Instant eventtime, Instant processingTime) {
        List<Object> builders = new ArrayList<>();
        ...
        //Check if interval 30 ids are published
        assertTrue(argContainsIds(argInput, Arrays.asList(id_71, id_72, id_73)));
        //missingIds contains no id
        assertTrue(argMissingIds.getValue().size() == 0);
    }
     ...
     * @param intervalEnd
     */
    public void testStep3Completeness(Instant timeoutEnd, Instant intervalEnd) {
     ...
     */
    public void testStep4Completeness(Instant eventtime, Instant processingTime) {
        List<Object> builders = new ArrayList<>();
        Map<String, InfraState> infraStates = new HashMap<>();
        ...
        ArgumentCaptor<Map<String, InfraState>> infraStatesArg = ArgumentCaptor.forClass(Map.class);
        ArgumentCaptor<Map<String, SingleVehicleData>> slowestVehData = ArgumentCaptor.forClass(Map.class);
        //No more new publish's - next publish at interval end
        Mockito.verify(pubMock, Mockito.times(2)).publish(argInput.capture(), argMissingIds.capture(), infraPara.capture(),
                infraStatesArg.capture(), slowestVehData.capture());
    }
     ...
     * @param intervalEnd
     */
    public void testStep5Completeness(Instant eventtime, Instant intervalEnd) {
        Map<String, InfraState> infraStates = new HashMap<>();
        List<String> i15Ids = Arrays.asList(id_71, id_72, id_73);
        List<String> i30Ids = Arrays.asList(id_75, id_76, id_77,
                                            id_79, id_80, id_81,
        ...
        ArgumentCaptor<Map<String, InfraState>> infraStatesArg = ArgumentCaptor.forClass(Map.class);
        ArgumentCaptor<Map<String, SingleVehicleData>> slowestVehData = ArgumentCaptor.forClass(Map.class);
        Mockito.verify(pubMock, Mockito.times(3)).publish(argInput.capture(), argMissingIds.capture(), infraPara.capture(),
                infraStatesArg.capture(), slowestVehData.capture());
     ...
     * @param timeoutEnd
     */
    public void testStep6Completeness(Instant intervalEnd, Instant timeoutEnd) {
     ...
     * @param processingTime
     */
    public void testStep7Completeness(Instant eventtime15, Instant eventtime30, Instant processingTime) {
        List<Object> builders = new ArrayList<>();
        Map<String, InfraState> infraStates = new HashMap<>();
        ...
        infraStates.put(id_76, new InfraState(id_76, true, null, eventtime30, false, true));
        ...
        ArgumentCaptor<Map<String, InfraState>> infraStatesArg = ArgumentCaptor.forClass(Map.class);
        ArgumentCaptor<Map<String, SingleVehicleData>> slowestVehData = ArgumentCaptor.forClass(Map.class);
        Mockito.verify(pubMock, Mockito.times(4)).publish(argInput.capture(), argMissingIds.capture(), infraPara.capture(),
                infraStatesArg.capture(), slowestVehData.capture());
     ...
     * @param processingTime
     */
    public void testStep8Completeness(Instant eventtime30, Instant processingTime) {
        List<Object> builders = new ArrayList<>();
        ...
        ArgumentCaptor<Map<String, InfraState>> infraStatesArg = ArgumentCaptor.forClass(Map.class);
        ArgumentCaptor<Map<String, SingleVehicleData>> slowestVehData = ArgumentCaptor.forClass(Map.class);
        Mockito.verify(pubMock, Mockito.times(5)).publish(argInput.capture(), argMissingIds.capture(), infraPara.capture(),
                infraStatesArg.capture(), slowestVehData.capture());
     ...
     * @param timeoutEnd
     */
    public void testStep9Completeness(Instant intervalEnd, Instant timeoutEnd) {
        //trigger interval timeout for interval 15 at second 27
        algo.intervalTimoutTrigger(timeoutEnd, intervalEnd, Arrays.asList(secondsI15, secondsI30));
