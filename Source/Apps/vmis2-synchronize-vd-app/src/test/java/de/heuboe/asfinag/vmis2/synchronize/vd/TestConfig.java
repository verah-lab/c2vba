...
class TestConfig {
    ...
    void init()  throws Exception  {
        ...
        GetAllItemsRequest reqVDEs = PGetAllItemsRequest.to(PGetAllItemsRequest.builder()
                .type(PConfigItemType.VDE_SENSOR).uzId("WIE").build());
        ...
        GetAllItemsRequest reqRSTs = PGetAllItemsRequest.to(PGetAllItemsRequest.builder()
                .type(PConfigItemType.RST).uzId("WIE").childOptsList(Arrays.asList(childOpt)).build());
        ...
        // Read locical passivation from file
        algoParaBuilder = ParameterSetList.newBuilder();
        ...
        JsonFormat.parser().merge(json, algoParaBuilder);
        algoPara = PParameterSetList.from(algoParaBuilder.build());
        assertNotNull(algoPara);

        // Send locical passivation
        topic = appProperties.getParameterRoadTopics().get(0).replace("{systemWideShortcut}",
                appProperties.getSystemWideShortcut());
        kafkaTemplate.send(MessageBuilder.withPayload(algoPara)
                .setHeader(KafkaHeaders.TOPIC, topic)
                .setHeader(KafkaHeaders.KEY, "LogischePassivierungVDE-WIE-INSTANZ1-A23_2")
