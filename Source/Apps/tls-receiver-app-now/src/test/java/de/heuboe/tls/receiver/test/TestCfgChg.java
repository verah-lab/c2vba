...
@ContextConfiguration( classes = { 
      de.heuboe.tls.receiver.test.config.Vmis2SystemMessageManagementProperties.class,
      ...
      de.heuboe.tls.receiver.test.MockedCfgGetterChg.class, // A2/A23

      de.heuboe.tls.receiver.test.config.ConfChg.class,
      de.heuboe.tls.receiver.test.config.TransformationRulesConfig.class
      } )
...
class TestCfgChg  {
    ...
    void chgCfgTest() throws Exception {
        ...
        while (
                ...
                ||
                ((2 - eps) > (cnt2 = meterRegistry.find( "DataObjects.sended" ).counter().count()))) {
            System.out.println( "Count of telegrams: " + cnt1 + " count of sended Objects " + cnt2 );
            Thread.sleep( 500 );
        }
        System.out.println( "-- Count of telegrams: " + cnt1 + " count of sended Objects " + cnt2 );
        
        assertTrue( Math.abs( cnt1 - 4. ) < eps );
        assertTrue( Math.abs( cnt2 - 2. ) < eps );

        cfgGetter.switchConfig( "tlsb-vers2.json", null, "tlsb-devs2.json", "tlsb-cabs2.json" ); // add a virtual de
        ...
        /* init change */ {
            ...
            kafkaTemplate.send( MessageBuilder.withPayload(changes).setHeader(KafkaHeaders.TOPIC, chgTopic).setHeader(KafkaHeaders.MESSAGE_KEY, "WIE").build() );
        }
       ...
       while (
               ...
               ||
               ((6 - eps) > (cnt2 = meterRegistry.find( "DataObjects.sended" ).counter().count()))) {
           System.out.println( "Count of telegrams: " + cnt1 + " count of sended Objects " + cnt2 );
           Thread.sleep( 500 );
       }
       System.out.println( "-- Count of telegrams: " + cnt1 + " count of sended Objects " + cnt2 );
       ...
       assertTrue( Math.abs( cnt1 - 8. ) < eps );
       assertTrue( Math.abs( cnt2 - 6. ) < eps ); // all objects of all telegrams have been understood
