...
public class TransformationRules {
	...
	private static final Logger LOGGER = Logger.getLogger(TransformationRules.class);
	
	private Map<Integer, List<DeBlockDefinition>> definitions = new HashMap<>();
	 ...
	 */
	public List<DeBlockDefinition> getDefinition(int fg, int id, int typ) {
	 ...
	 */
	public void addDefinition(DeBlockDefinition def, ParserRuleContext ctx) throws TlsReceiverException {
		for(Integer fg : def.getFg()) {
			for(Integer id : def.getId()) {
				int typ = def.getTyp();
				int key = (fg << 16) + (id << 8) + typ;
				List<DeBlockDefinition> l = definitions.get(key);
				if (null != l) {
         ...
         private void checkUseOfLocationNames( Integer fg, Integer id, int typ, List<DeBlockDefinition> l, ParserRuleContext ctx ) throws TlsReceiverException {
                for ( DeBlockDefinition ll : l ) {
                        if (null == ll.getSpecialLocationsName() && null == ll.getSpecialLocationsExcludedName()) {
                                throw new TlsReceiverException("duplicate (no locations used) Fg/Id/Typ: " + fg + "/" + id + "/" + typ + " " + Parser.inputLocation( ctx ) );
                        }
                }
        }
