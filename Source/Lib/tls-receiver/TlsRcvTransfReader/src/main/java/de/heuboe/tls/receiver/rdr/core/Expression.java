...
import de.heuboe.tls.receiver.interfaces.ExpressionInterface;
import de.heuboe.tls.receiver.rdr.item.FloatItem;
import de.heuboe.tls.receiver.rdr.item.IntegerItem;
import de.heuboe.tls.receiver.rdr.item.IntegerListItem;
...
public class Expression implements ExpressionInterface {
	...
	private DataItemType resType = null;
	
	static final String INT_ONLY = "integers or float values expected only";
...
//	
	...
	public Expression(int value) {
         ...
         * @param valList A list constant values
         */
        ...
        /**
         * Evaluate the resulting type for this expression
         */
        @Override
        public void prepareType( String name, Map<String, DataItemType> typeMap ) {
                ...
                if ( variable != null ) {
                        resType = typeMap.get( variable );
                        return;
                }
                if (this.constValList != null) {
                        resType = DataItemType.ILIST;
                        return;
                }
                ex1.prepareType( name, typeMap );
                ex2.prepareType( name, typeMap );
                DataItemType leftItemType = ex1.getType();
                DataItemType rightItemType = ex2.getType();
                if ( leftItemType == null || rightItemType == null ) {
                        throw new IllegalArgumentException( "unresolvable expression type" );
                }
                switch ( operator ) {
                case "+":
                        resType = addSubMultItemsType( leftItemType, rightItemType );
                        break;
                case "-":
                        resType = addSubMultItemsType( leftItemType, rightItemType );
                        break;
                case "*":
                        ...
                        break;
                case "/":
                        resType = divideItemsType( leftItemType, rightItemType );
                        break;
                case "&":
                        resType = integerOnlyIntegerResType( leftItemType, rightItemType );
                        break;
                case "|":
                        resType = integerOnlyIntegerResType( leftItemType, rightItemType );
                        break;
                ...
                case "||":
                        ...
                        break;
                case ">":
                        resType = orderCompareItemsType( leftItemType, rightItemType );
                        break;
                case ">=":
                        resType = orderCompareItemsType( leftItemType, rightItemType );
                        break;
	...
	private DataItem multiplyItems(DataItem leftItem, DataItem rightItem) {
        ...
        private DataItemType addSubMultItemsType( DataItemType leftItem, DataItemType rightItem ) {
                checkType( leftItem, rightItem );
                if ( leftItem == DataItemType.INTEGER && rightItem == DataItemType.INTEGER ) {
	...
	private DataItem divideItems(DataItem leftItem, DataItem rightItem) {
        ...
        private DataItemType divideItemsType( DataItemType leftItem, DataItemType rightItem ) {
                checkType( leftItem, rightItem );
                return DataItemType.FLOAT;
        }
	...
	private DataItem logicOrItems(DataItem leftItem, DataItem rightItem) {
        ...
        private DataItemType integerOnlyIntegerResType(DataItemType leftItem, DataItemType rightItem ) {
                checkType(leftItem, rightItem);
                if (leftItem != DataItemType.INTEGER || rightItem != DataItemType.INTEGER) {
                        throw new IllegalArgumentException(ONLY_INTEGERS);
                }
...
// ------------------------------------
        ...
        private DataItemType orderCompareItemsType(DataItemType leftItem, DataItemType rightItem) {
                checkType(leftItem, rightItem);
                return DataItemType.INTEGER;
        }
...
//-------------------------------------	
        ...
        private void check( DataItem leftItem, DataItem rightItem ) {
                if ( leftItem.getType() != DataItemType.INTEGER && leftItem.getType() == DataItemType.FLOAT ) {
                        throw new IllegalArgumentException( INT_ONLY );
                }
        ...
        private void checkType( DataItemType leftItem, DataItemType rightItem ) {
                if ( leftItem != DataItemType.INTEGER && leftItem == DataItemType.FLOAT ) {
                        throw new IllegalArgumentException( INT_ONLY );
                }
                if ( rightItem != DataItemType.INTEGER && rightItem == DataItemType.FLOAT ) {
                        throw new IllegalArgumentException( INT_ONLY );
                }
        }
}
