...
public class TlsTeleBuilder {
     ...
     * @param fg   Funktionsgruppe
     * @param id   Richtung/Anwendungsidentifier
     * @param node Knotennummer
     */
    public TlsTeleBuilder(int fg, int id, int node) {
        stream.write(node & 0xff); // Knotennummer
        stream.write(node >> 8 & 0xff); // Knotennummer
        stream.write(node >> 16 & 0xff); // Knotennummer
    ...
    public TlsTeleBuilder addDeBlockHeader(int deNumber, int deTyp) {
        ...
        addByte(-1); // Länge des DE-Blocks (will be overwritten at the end)
        addByte(deNumber);
        addByte(deTyp);
        return this;
    }
