...
echo "Applying c2vba-synchronize-vd-app"
kubectl create cm c2vba-synchronize-vd-app --from-file=configs --dry-run=true -o yaml | kubectl apply -n $NAMESPACE -f -
kubectl apply -n $NAMESPACE -f c2vba-synchronize-vd-app-service.yaml
kubectl apply -n $NAMESPACE -f c2vba-synchronize-vd-app-deployment.yaml
