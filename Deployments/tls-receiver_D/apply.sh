...
echo "Applying tls-receiver"
kubectl create cm tls-receiver --from-file=configs --dry-run=true -o yaml | kubectl apply -n $NAMESPACE -f -
kubectl apply -n $NAMESPACE -f tls-receiver-service.yaml
kubectl apply -n $NAMESPACE -f tls-receiver-deployment.yaml
